...
def train_cl(args, config, model, train_datasets, test_datasets, iters=2000, batch_size=32, baseline='none',
    ...
    '''
    logging.basicConfig(filename='log/ER_CIFAR10_wo_AQA.log', level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')
    logging.info('111')
    ...
    for context, train_dataset in enumerate(train_datasets, 1):
        ...
        if checkattr(model, 'use_memory_buffer'):
            ...
            samples_per_class = int(np.floor((model.budget_per_class*len(train_datasets))/context))
        ...
        print('\n\n' + 'CONTEXT:%d EVALUATION '.center(35, '*') %context)

        accs = []
        connection = []
        forgetting_measure = []
        # Evaluate accuracy of current model on current test-set
        print("\n Accuracy of context:%d model on test-set:" %context)
        for i in range(context):
            ...
            accs.append(acc)
            if i + 1 != context:
                forgetting_measure.append(max_acc[i] - acc)
                max_acc[i] = max(max_acc[i], acc)
                connection.append(last_acc[i] - acc)
            else:  
                max_acc.append(acc)

        if context == 1:
            forgetting_measure.append(1)
        else:
            connection_tensor = torch.tensor(connection, dtype=torch.float32) 
            softmax_values_connection = torch.nn.functional.softmax(connection_tensor, dim=0) 

            forgetting_measure_context = 0
            for i in range(context):
                if i + 1 == context:
                    break
                forgetting_measure_context += softmax_values_connection[i] * forgetting_measure[i]
        ...
        if target == True:
            ...
            for class_id in new_classes:
                ...
                # based on this dataset, construct new memory-set for this class
                allowed_classes = active_classes[-1] if per_context and not per_context_singlehead else active_classes
                # print(len(class_dataset),(model.budget_per_class*len(train_datasets))*(forgetting_measure[int(class_id/model.classes_per_context)]/sum(forgetting_measure)))
                print(softmax_values[int(class_id/model.classes_per_context)])
                # if(class_id < model.classes_per_context*context - 1) and connection[class_id] > 0.3:
                #     model.construct_memory_set2(dataset=class_dataset, n=int((model.budget_per_class*len(train_datasets))*(softmax_values[int(class_id/model.classes_per_context)])), label_set=allowed_classes, featuresmean = features[-1])
                # else:
                #     model.construct_memory_set(dataset=class_dataset, n=int((model.budget_per_class*len(train_datasets))*(softmax_values[int(class_id/model.classes_per_context)])), label_set=allowed_classes)
                #model.construct_memory_set3(dataset=class_dataset, n=int((model.budget_per_class*len(train_datasets))*(softmax_values[int(class_id/model.classes_per_context)])), label_set=allowed_classes)
                #print(model.budget_per_class)
                model.construct_memory_set3(dataset=class_dataset, n=model.budget_per_class, label_set=allowed_classes)
            model.compute_means = True
            # print(len(model.memory_sets))
...
def train_fromp(model, train_datasets, iters=2000, batch_size=32,
    ...
    for context, train_dataset in enumerate(train_datasets, 1):
        ...
        if checkattr(model, 'use_memory_buffer'):
            ...
            #samples_per_class = np.floor((model.budget_per_class*len(train_datasets))/context)
